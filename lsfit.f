      subroutine lsfit(lb,dcor,rms,nind)
      include 'curves_data.inc'
      real*8 h(3,3),k(3,3)
      character*1 base,type
      character*4 mnam,munit,ban
      character*8 ibnam
      logical*2 nind
      dimension cg1(3),cg2(3),u(3,3),w(21),v(6,6),dcor(40,3),nind(20)
...
c-------------------------------------------------------find ls rotation
      ...
      do i=1,3
      do j=1,3
      u(i,j)=u(i,j)+(bref(lb,l,i)-cg2(j))*(dcor(l,j)-cg1(j))/kb
      enddo
      enddo
      endif
      enddo
...
c-------------------------------------------------------make ls rotation
      ...
      do l=1,ibref(lb)
      if(nind(l)) then
      x0=bref(lb,l,1)-cg2(1)
      y0=bref(lb,l,2)-cg2(2)
      z0=bref(lb,l,3)-cg2(3)
